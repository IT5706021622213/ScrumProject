            ...
            this.comment_data = val.data;
              if(this.todoid[h] == this.comment_data[h].id){
                  ...
                  for (let i = 0 ; i < this.comment_data[h].comments.data.length ; i++) {
                    ...
                    if(this.comment_data[h].comments.data[i].message == '#start' && this.comment_data[h].comments.data[i].message != '#end'){
                      if (this.comment_data[h].id == this.todoid[h]) {
                        let newinprogress = {
                          status: 'กำลังทำ',
                          id: this.comment_data[h].id,
                          name: this.todoposts[h].message
                        }
                        this.inprogressposts.push(newinprogress);
                        console.log(JSON.stringify(this.inprogressposts));
                      }
                    }
